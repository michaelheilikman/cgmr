...
<div class="container px-md-5 py-md-5">
    ...
    <div class="content col-12 d-flex flex-wrap ps-0">
        ...
        <div class="socialNav d-flex d-md-block col-12 col-lg-1 h-25 justify-content-center position-md-sticky sticky-md-top ps-0">
            <a href="https://www.linkedin.com/sharing/share-offsite/?url=<?php echo $path.'project/'.$event->url ?>" target="_blank" class="border border-success border-2 rounded-3 bg-white text-success me-2 me-md-0"><i class="bi bi-linkedin position-relative"></i></a>
            <a href="http://www.twitter.com/share?url=<?php echo $path.'project/'.$event->url ?>&amp;text=<?php echo $event->title ?>&amp;hashtags=<?php echo $event->from_site  ?>" target="_blank"  class="border border-success border-2 rounded-3 bg-white text-success me-2 me-md-0"><i class="bi bi-twitter"></i></a>
            <a href="https://www.facebook.com/sharer/sharer.php?u=<?php echo $path.'project/'.$event->url ?>" target="_blank"  class="border border-success border-2 rounded-3 bg-white text-success me-2 me-md-0"><i class="bi bi-facebook"></i></a>
            <a href="mailto:?subject=<?php echo $event->from_site.' | '.$event->title ?>&body=Bonjour {*NOM DU CORRESPONDANT*},%0D%0A%0D%0AJe souhaiterai partager avec vous une page du site <?= $website ?> :%0D%0A<?php echo $path.'project/'.$event->url ?>%0D%0A%0D%0A Retrouvez toutes les informations sur le site <?php echo $website ?> <?php echo $path ?>%0D%0A%0D%0A Très respectueusement !" target="_blank"  class="border border-success border-2 rounded-3 bg-white text-success"><i class="bi bi-send-fill"></i></a>
        </div>
